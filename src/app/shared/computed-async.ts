...
// without options
export function computedAsync<T>(computeFn: (previousValue?: T) => Observable<T> | Promise<T> | T | undefined): Signal<T | undefined>;
...
function flattenObservable<T>(source: Subject<Promise<T> | Observable<T>>, behavior: ComputedAsyncBehavior): Observable<T> {
  const behaviorMap = {
    switch: switchAll,
    merge: mergeAll,
    concat: concatAll,
