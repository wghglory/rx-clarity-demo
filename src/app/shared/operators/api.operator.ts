 ...
 * data$ = this.shopService.products$.pipe(
 ...
 switchMap(() => {
      return merge(
        of({ loading: true, error: undefined, data: undefined }),
        this.service.apiCall().pipe(
          map(data => ({ loading: false, error: undefined, data })),
          tap({
            next: res => callback?.(res.data),
            error: err => errorCallback?.(err),
          }),
          catchError(error => of({ loading: false, error, data: undefined })),
        ),
      )
    })
 )
...
export function api<T>(
  ...
  return pipe(
    ...
    startWith({ loading: true, error: undefined, data: undefined }),
